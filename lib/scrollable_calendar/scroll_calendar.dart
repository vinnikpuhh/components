...
// ignore_for_file: unnecessary_null_comparison, must_be_immutable, avoid_print, unused_element, unused_local_variable
...
///
Future<DateTime?> showDatePickerFork({
...
class _DatePickerDialogState extends State<DatePickerDialog>
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? dateStyle =  textTheme.bodyMedium?.copyWith(color: onPrimarySurface);
...
class _DatePickerHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? helpStyle = textTheme.titleMedium?.copyWith(
...
///
Future<DateTimeRange?> showDateRangePickerFork({
  required BuildContext context,
...
class DateRangePickerDialog extends StatefulWidget {
  ...
  final TextStyle rangeDateTextStyle;
    ...
    switch (_entryMode.value) {
      ...
      case DatePickerEntryMode.calendarOnly:
        contents = CalendarRangePickerDialog(
          ...
          rangeDateTextStyle: widget.rangeDateTextStyle,
...
class CalendarRangePickerDialog extends StatefulWidget {
  CalendarRangePickerDialog({
    Key? key,
    required this.rangeDateTextStyle,
    required this.size,
  ...
  @override
  State<CalendarRangePickerDialog> createState() =>
      _CalendarRangePickerDialogState();
}

class _CalendarRangePickerDialogState extends State<CalendarRangePickerDialog> {
  List month = [
    'января',
    'февраля',
    'марта',
  ...
  @override
  Widget build(BuildContext context) {
    String start = widget.startDay?.toString() != '' && widget.startDay != null
        ? '${widget.startDay?.day ?? ''} ${month[widget.startDay!.month - 1]} ${widget.startDay!.year}'
        : '';

    String end = widget.endDay?.toString() != '' && widget.endDay != null
        ? '${widget.endDay?.day ?? ''} ${month[widget.endDay!.month - 1]} ${widget.endDay!.year}'
        : '';
    return  Column(
          children: [
            Stack(
              children: [
                ...
                Align(
                  ...
                  child: GestureDetector(
                    onTap: () {
                      Navigator.pop(context);
                      /* BlocProvider.of<MainCubit>(context).endDate = null;
                  BlocProvider.of<MainCubit>(context).startDate = null; */
                    },
                    behavior: HitTestBehavior.opaque,
                    child: Padding(
                      padding: EdgeInsets.only(
                        top: widget.size.height * 0.055,
            ...
            Expanded(
              child: Column(
                children: [
                  ...
                  widget.startDay.toString() == '' || widget.startDay == null
                      ? Text(
                          'Выберите дату начала',
                          style: widget.rangeDateTextStyle,
                        )
                      : Align(
                          alignment: Alignment.center,
                          child: Row(
                  ...
                  Expanded(
                    child: CalendarDateRangePicker(
                      initialStartDate: widget.startDay,
                      initialEndDate: widget.endDay,
                      firstDate: widget.firstDate,
                      lastDate: widget.lastDate,
                      currentDate: widget.currentDate,
            ...
            GestureDetector(
              behavior: HitTestBehavior.opaque,
              onTap: () {
                /*  BlocProvider.of<MainCubit>(context).state.endDate = widget.endDay;
                BlocProvider.of<MainCubit>(context).state.startDate =
                    widget.startDay; */
                Navigator.pop(context);
                /* widget.filterCallBack(endDay: widget.endDay, startDay: widget.startDay); */
              },
              child: Container(
                height: 60,
                color: Colors.green,
...
class _MonthItemState extends State<_MonthItem> {
  ...
  Widget _buildDayItem(BuildContext context, DateTime dayToBuild,
    ...
    if (isSelectedDayStart || isSelectedDayEnd) {
      ...
      itemStyle = textTheme.bodyMedium?.apply(color: Colors.white);
        ...
        Container(
          ...
          child: ExcludeSemantics(
            child: Text(
              ...
              style:
                 const TextStyle() /* textTheme.bodyText2!
  ...
  Widget build(BuildContext context) {
    ...
    final Color onPrimarySurfaceColor =
        colorScheme.brightness == Brightness.light
            ? colorScheme.onPrimary
            : colorScheme.onSurface;
    final TextStyle? dateStyle =  textTheme.bodyMedium?.apply(color: onPrimarySurfaceColor)/* orientation == Orientation.landscape
